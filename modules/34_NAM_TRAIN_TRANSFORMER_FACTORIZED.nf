...
process M34_NAM_TRAIN_TRANSFORMER_FACTORIZED {
    ...
    tuple val(family), val(configId), val(arm), val(radiusCm), val(radiusToken),
          ...
          path("m34_train_${family}_${configId}_${taskToken}_${arm}/transformer_batching.receipt.json"),
          emit: trained

    script:
    """
    set -euo pipefail
    test '${family}' = transformer_small
    mkdir -p staged/bin
    cp ${transformerTrainerPy} staged/bin/m34_train_transformer_factorized.py
    ...
    PYTHONPATH=staged/bin python3 staged/bin/m34_train_transformer_factorized.py \
    ...
    touch \
      ...
      m34_train_${family}_${configId}_${taskToken}_${arm}/transformer_batching.receipt.json
    """
}
