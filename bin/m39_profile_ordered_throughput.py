...
SOURCE_FILES = ("m39_profile_ordered_throughput.py", "m39_throughput_sampling.py", *HISTORICAL_SOURCE_FILES)
...
def load_profile(path: Path) -> dict:
    ...
    require(isinstance(cfg, dict) and set(cfg) == PROFILE_FIELDS, "throughput profile inventory differs")
    require(cfg["schema_version"] == SCHEMA and cfg["scope"] == SCOPE, "throughput scope differs")
    ...
    require(cfg["device"] == "cpu", "only the existing CPU runtime is authorized")
...
def measure_step(store, model, optimizer, row: dict, case: dict, cfg: dict,
    ...
    batch = pack_batch(store, pairs, max_input_bytes=cfg["max_input_bytes"], device=cfg["device"])
    ...
    zero_grad_seconds = time.perf_counter() - tick
    ...
    if policy == "grouped":
        ...
        result["grouped_fullpass_estimate"] = {
            **estimate, "seconds": estimate["estimated_grouped_pass_seconds"],
            ...
            "scope": "same_CPU_small_recipe_grouped_policy_audited_steps_only",
...
def run_case(store_dir: Path, parent_path: Path, folds_path: Path, profile_path: Path,
    ...
    report = {"schema_version": SCHEMA, "decision": DECISION, "case": case,
