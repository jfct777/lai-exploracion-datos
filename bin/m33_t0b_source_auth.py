#!/usr/bin/env python3
"""Authenticate the exact committed sources used by M33 T0b."""
...
REQUIRED_SOURCES = {
    ...
    "bin/m33_t0a_forward.py", "bin/m33_t0b_preflight.py", "bin/m33_t0b_forward.py",
    "bin/m33_t0b_compare.py", "bin/m33_t0b_source_auth.py",
    "conf/m33_pre4_preregistration.json", "conf/m33_t0b_contract.json",
    "conf/m33_t0b.config", "modules/33_T0B_FULL_CHR22.nf", "workflows/m33_t0b.nf",
    "containers/m33-t0a/Dockerfile", "tests/test_m33_t0b.py",
}
...
def main() -> int:
    ...
    require(set(sources) == REQUIRED_SOURCES, "T0b source inventory is incomplete")
    ...
    dirty = subprocess.run(["git", "-C", str(args.repository_root), "status", "--porcelain",
                            "--", *sorted(sources)], check=True, capture_output=True,
                           text=True).stdout
    require(not dirty.strip(), "T0b sources are dirty or untracked")
    hashes = {}
    for relative, staged in sorted(sources.items()):
        committed = subprocess.run(["git", "-C", str(args.repository_root), "show",
                                    f"{args.git_commit}:{relative}"], check=True,
