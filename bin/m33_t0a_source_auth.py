#!/usr/bin/env python3
"""Authenticate the exact committed sources used by M33 T0a."""
...
def main() -> int:
    ...
    require(set(sources) == REQUIRED_SOURCES, "T0a source inventory is incomplete")
    ...
    dirty = subprocess.run(["git", "-C", str(args.repository_root), "status", "--porcelain",
                           ...
                           text=True).stdout
    require(not dirty.strip(), "T0a sources are dirty or untracked")
    hashes = {}
    ...
    write_exclusive(args.output, {
        "stage": "M33_T0A_SOURCE_AUTH", "status": "PASS_EXACT_COMMIT_AND_SOURCE_HASHES",
