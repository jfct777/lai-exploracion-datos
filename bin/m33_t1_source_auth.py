#!/usr/bin/env python3
"""Authenticate the exact committed sources used by M33 T1."""
...
def main() -> int:
    ...
    require(set(sources) == REQUIRED_SOURCES, "T1 source inventory is incomplete")
    head = subprocess.run(
    ...
    require(not dirty.strip(), "T1 sources are dirty or untracked")
    hashes = {}
    for relative, staged in sorted(sources.items()):
        committed = subprocess.run(
            ["git", "-C", str(args.repository_root), "show",
